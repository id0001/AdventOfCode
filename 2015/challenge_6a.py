...
instructions = list()
matrix = [False for i in range(1000*1000)]
...
def turn_on(matrix, start, end):
    for y in range(start.y, end.y):
        for x in range(start.x, end.x):
            matrix[matrix_index(x, y)] = True


def turn_off(matrix, start, end):
    for y in range(start.y, end.y):
        for x in range(start.x, end.x):
            matrix[matrix_index(x, y)] = False


def toggle(matrix, start, end):
    for y in range(start.y, end.y):
        for x in range(start.x, end.x):
            matrix[matrix_index(x, y)] = not matrix[matrix_index(x, y)]
...
print_solution(len(list(filter(lambda x: x, matrix))))
