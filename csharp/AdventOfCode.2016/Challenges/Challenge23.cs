...
public class Challenge23(IInputReader inputReader)
{
    ...
    private static void RunProgram(string[][] program, Dictionary<string, int> registers)
    {
        ...
        {
            if (ip + 5 < program.Length && program[ip + 5][0] == "jnz" && program[ip + 5][2] == "-5" &&
                !int.TryParse(program[ip][1], out _))
            {
            ...
            ip = program[ip] switch
            {
                ...
                ["jnz", var x, var y] when int.TryParse(x, out var ix) && int.TryParse(y, out var iy) => ix != 0
                    ? ip + iy
                    : ip + 1, // jnz 1 3
                ["jnz", var x, var y] when int.TryParse(x, out var ix) => ix != 0
                    ? ip + registers[y]
                    : ip + 1, // jnz 3 a
                ["jnz", var x, var y] when int.TryParse(y, out var iy) => registers[x] != 0
                    ? ip + iy
                    : ip + 1, // jnz a 2
    ...
    private static void Multiply(string[][] program, Dictionary<string, int> registers, int ip)
    {
        ...
        */
        ...
        var resultReg = program[(ip + 1)..(ip + 6)].Single(inst =>
            inst[0] != "jnz" && inst[1] != valueReg && inst[1] != multiplierReg && inst[1] != between)[1];
