...
public class Challenge10(IInputReader inputReader)
{
    ...
    private static readonly Regex ComparePattern = new(@"bot (\d+) gives low to (output|bot) (\d+) and high to (output|bot) (\d+)");
    ...
    public async Task<string> Part1Async()
    {
        ...
        var botMemory = compareNodes.ToDictionary(kv => kv.Key, kv => new List<int>());
    ...
    private static Node ParseLine(string line) => (InputPattern.Match(line), ComparePattern.Match(line)) switch
    {
        ...
        (_, var c) when c.Success => new CompareNode(c.Groups[1].Value.As<int>(), c.Groups[2].Value, c.Groups[3].Value.As<int>(), c.Groups[4].Value, c.Groups[5].Value.As<int>()),
