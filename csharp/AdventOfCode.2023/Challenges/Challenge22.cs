...
public class Challenge22
{
    ...
    {
        var bricks = await _inputReader.ParseLinesAsync(22, ParseLine).Select((v, i) => new Brick(i, v)).ToListAsync();
        var graph = CreateSupportGraph(bricks);
        
        return bricks.Count(b => !graph.OutEdges(b.Id).Any() || graph.OutEdges(b.Id).Keys.All(s => graph.InEdges(s).Count > 1)).ToString();
    }
    ...
    private static DirectedGraph<int,int> CreateSupportGraph(IEnumerable<Brick> cubes)
    {
        ...
        foreach(var brick in cubes.OrderBy(b => b.Volume.Front))
        {
            ...
            foreach(var support in visited.Where(v =>v.Volume.IntersectsWith(volume with {Z = volume.Position.Z-1})))
