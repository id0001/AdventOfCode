...
public class Challenge12
{
    ...
    {
        return await _inputReader.ParseLinesAsync(12, ParseLine).SumAsync(x => CountPossibilities(new Dictionary<(int, int, int), long>(), x.Item1, x.Item2, 0, 0, 0)).ToStringAsync();
    }

    [Part2]
    public async Task<string?> Part2Async()
    {
        return await _inputReader.ParseLinesAsync(12, ParseLine2).SumAsync(x => CountPossibilities(new Dictionary<(int, int, int), long>(), x.Item1, x.Item2, 0, 0, 0)).ToStringAsync();
    }

    private long CountPossibilities(Dictionary<(int, int, int), long> cache, string springs, int[] groups, int si, int gi, int captureLength)
    {
        ...
        if (springs[si] is '.' or '?') // Assuming '?' is '.'
        {
            if (captureLength == 0) // Before a capture
                possibilities += CountPossibilities(cache, springs, groups, si + 1, gi, 0); // Not in capture. Skip to next char
            else if (captureLength == groups[gi]) // After a capture
                possibilities += CountPossibilities(cache, springs, groups, si + 1, gi + 1, 0); // Captured group. Go to next group
        }
    ...
    private (string, int[]) ParseLine2(string line)
    {
        ...
        var first = string.Join("?", new[] { parts.First, parts.First, parts.First, parts.First, parts.First });
