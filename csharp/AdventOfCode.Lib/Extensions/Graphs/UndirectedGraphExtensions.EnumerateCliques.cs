...
public static partial class UndirectedGraphExtensions
{
    ...
    private static void BronKerbosch<TVertex, TEdge>(UndirectedGraph<TVertex, TEdge> graph, List<List<TVertex>> cliques,
        ImmutableHashSet<TVertex> r, ImmutableHashSet<TVertex> p, ImmutableHashSet<TVertex> x)
        where TVertex : notnull
        where TEdge : notnull
    {
        ...
        {
            BronKerbosch(graph, cliques, r.Add(v), p.Intersect(graph.AdjacentEdges(v).Keys),
                x.Intersect(graph.AdjacentEdges(v).Keys));
            p = p.Remove(v);
            x = x.Add(v);
        }
    }
