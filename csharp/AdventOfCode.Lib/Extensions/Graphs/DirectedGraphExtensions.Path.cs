...
{
    public static AStar<DirectedGraph<TVertex, int>, TVertex> Path<TVertex>(this DirectedGraph<TVertex, int> graph,
        TVertex start)
        where TVertex : notnull
        => new(graph, start, n => GetAdjacent(graph, n), (c, n) => GetWeight(graph, c, n), _ => 0);
