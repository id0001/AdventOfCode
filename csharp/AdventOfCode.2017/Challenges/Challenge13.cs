...
public class Challenge13(IInputReader inputReader)
{
    ...
    public async Task<string> Part1Async() => await inputReader.ParseLinesAsync(13, ParseLine)
        .AggregateAsync(0,
            (score, shell) =>
                score + (GetPositionInTime(shell.Layer, shell.Depth) == 0 ? shell.Layer * shell.Depth : 0))
        .ToStringAsync();
    ...
    private static (int Layer, int Depth) ParseLine(string line) =>
        line.SplitBy(":").As<int>().Into(parts => (parts.First(), parts.Second()));
