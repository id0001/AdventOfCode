...
public class Challenge21(IInputReader inputReader)
{
    ...
    {
        var enhancements = await inputReader.ParseLinesAsync(21, ParseLine).SelectMany(x => x.ToAsyncEnumerable()).ToDictionaryAsync(kv => kv.Input, kv => kv.Output);
        var current = new Pattern(Start);

        var test = enhancements[current];

        for (var i = 0; i < 5; i++)
        {
            current = new Pattern(current.Subdivide().Select(x => enhancements[x]));
        }
    ...
    private static IEnumerable<Enhancement> ParseLine(string line)
    {
        ...
        return new[]
        {
            new Enhancement(new Pattern(input,0, false, false), new Pattern(output)),
            new Enhancement(new Pattern(input, 0, true, false), new Pattern(output)),
            ...
            new Enhancement(new Pattern(input, 90, false, false), new Pattern(output)),
            new Enhancement(new Pattern(input, 90, true, false), new Pattern(output)),
            ...
            new Enhancement(new Pattern(input, 180, false, false), new Pattern(output)),
            new Enhancement(new Pattern(input, 180, true, false), new Pattern(output)),
            ...
            new Enhancement(new Pattern(input, 270, false, false), new Pattern(output)),
            new Enhancement(new Pattern(input, 270, true, false), new Pattern(output)),
        ...
        public IEnumerable<Pattern> Subdivide()
        {
            ...
            else
            {
                for (var y = 0; y < split.Length; y += 3)
                {
                    ...
                    {
                        yield return new Pattern($"{split[y][x..(x + 3)]}/{split[y + 1][x..(x + 3)]}/{split[y + 2][x..(x + 3)]}");
                    }
                }
            }
        }

        public bool Equals(Pattern? other) => other is not null && other.Value._pattern == _pattern;

        public override int GetHashCode() => HashCode.Combine(_pattern);

        public override string ToString() => _pattern;

        private static string FlipHorizontally(string pattern) => string.Join("/", pattern.SplitBy("/").Select(s => string.Join("", s.Reverse())));
